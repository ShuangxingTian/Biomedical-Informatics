...
if __name__ == "__main__":
    ...
    # model_restoration = arch(img_size=input_size, embed_dim=16,depths=depths,
    #              win_size=8, mlp_ratio=4., qkv_bias=True, in_chans=1,
    #              token_projection='linear', token_mlp='leff',
    #              downsample=Downsample, upsample=Upsample, se_layer=False).cuda()
    model_restoration = Uformer(img_size=128, in_chans=1, embed_dim=8, win_size=8, token_projection='linear',
                    token_mlp='leff').cuda()
    summary(model_restoration, (1, 256, 256))
    # arch = LeWinformer    
    # depth = 20
