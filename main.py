...
def train():
    ...
    model = Uformer(img_size=64, in_chans=1, embed_dim=16, win_size=8, token_projection='linear', token_mlp='leff').to(device)
    batch_size = 1
...
def test():
    # model = Unet(1, 1)
    model = Uformer(img_size=64, in_chans=1, embed_dim=16, win_size=8, token_projection='linear', token_mlp='leff')
    model.load_state_dict(torch.load(PATH))
    test_dataset = TestDataset("dataset/test/original", transform=x_transforms,target_transform=y_transforms)
    ...
    with torch.no_grad():
        for index, x in enumerate(dataloaders):
            ...
            io.imsave("./dataset/test/predict/" + str(index) + "_predict.png", img)
            plt.pause(0.01)
        plt.show()
